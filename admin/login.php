...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  try {
    ...
    $stmt = $pdo->prepare("
      SELECT *
      FROM staffs
      WHERE (login_id = ? OR employee_no = ? OR LOWER(name) = LOWER(?))
        AND is_active = 1
      LIMIT 1
    ");
    ...
    if ($staff && password_verify($password, $hash)) {
      $_SESSION['auth_user'] = [
        'id' => (int)$staff['id'],
        'name' => $staff['name'],
        'role' => $staff['role'] ?: 'staff',
        'store_id' => (int)($staff['store_id'] ?? 1),
      ];
...
<head>
  ...
  <link rel="stylesheet" href="../assets/css/admin-pro.css?v=26">
  <link rel="stylesheet" href="../assets/css/os-pro.css?v=26">
</head>
<body class="os-login-body">
  <main class="os-login-card">
    ...
    <form method="post" class="os-form">
      <label>Login ID<input name="login_id" required placeholder="owner / kiho / yuina"></label>
